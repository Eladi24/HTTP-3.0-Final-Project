...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        help="Data to send in the request body (for POST requests)",
        default=None,
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--zero-rtt",
        action="store_true",
        help="Enable 0-RTT session resumption",
    )
    args = parser.parse_args()

    configuration = QuicConfiguration(is_client=True, alpn_protocols=[H3_ALPN])
    configuration.verify_mode = ssl.CERT_NONE
    configuration.load_verify_locations(cafile=os.path.join(os.path.dirname(__file__), "ssl_cert.pem"))
    # Use QUICv2
